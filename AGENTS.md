...
- Reader web 共享语义：`reader-content-stream.js` 是 reader 文本 normalization、matchable/raw
  计数、VN source stream、ruby-aware text entry、结构 ID 和 standalone media unit 的共享语义入口；
  `reader-range-map.js` 拥有 VN 渲染屏幕的 raw/highlight range、matchable Sasayaki range 和
  source-to-clone offset registration。VN 是特殊分页模式，不要重新引入 VN 私有的文本 offset、range、
  media 分类或 clone offset 实现。Paginated/continuous 只复用低风险纯文本语义 helper；除非先落成
  单独方案和验证计划，不要把它们的 page/scroll runtime 接到 content stream instance 或 range map。
