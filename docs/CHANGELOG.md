...
- Pause Sasayaki auto-scroll on skipped image pages for one second, including multiple image pages and image-only chapters, without leaving playback paused when the hold is interrupted.
- Keep Sasayaki cross-chapter cue jumps from briefly showing the chapter start or counting the hidden restore landing in reading statistics.
