...
- Keep Sasayaki playback owned by a service-backed MediaSession so background playback stays alive on background-restricted Samsung devices, reader exit stops playback, and notification or launcher return opens the active reader.
- Keep the Sasayaki notification's forward cue button working from system media controls.
