...
- Reader Appearance fonts: cancel and retry an offline recommended-font
  ...
  independently; restart and switch profiles to confirm per-family variant
  memory. Import two TTF/OTF variants of one family, replace an existing
  family/weight/style slot, confirm its old basename selection and dictionary
  CSS alias still resolve, delete the family, and verify WOFF/WOFF2 remain
  standalone while TTC is not offered. Recheck paginated, continuous, and VN
  rendering in horizontal/vertical writing plus Reader, Dictionary, and Process
  Text lookup popups.
