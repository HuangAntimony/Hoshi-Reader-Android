...
- Device-validate remaining reader highlight restore, jump, delete, all-color, continuous-mode, and real-device selection-toolbar behavior against iOS after paginated creation, primary Highlight toolbar placement, anchored color-picker placement, native selection drag page-locking, ruby/furigana-safe highlight saving, and ruby-edge native selection hit areas were verified on Android targets.
