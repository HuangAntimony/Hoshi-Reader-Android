...
- Paginated reader page turns now cache chapter page bounds after layout, update visible progress from memory immediately, debounce bookmark saves until page turning is idle, skip no-op selection bridge calls, and coalesce bookshelf refreshes until Reader close, reducing PageUp/PageDown and swipe jank on slower e-ink devices.
