...
- Sasayaki reader highlighting now keeps the non-E-ink inline WebView path for scrolling alignment, falls back to span highlighting for emphasized text so EPUB emphasis marks remain visible, dynamically switches colored inline highlights back on after E-ink Mode is disabled in an open reader, uses matching-thickness ruby-aware E-ink overlay lines plus reader root lookup boxes, keeps active cues stable through delayed match sidecar application, and no longer blocks reader WebView creation on Sasayaki match sidecar loading; blocked: device-validate horizontal/vertical furigana cues and lookup boxes on an E-ink target, including popup close, next lookup, navigation clearing, delayed restore/sidecar cue display, continuous-mode non-E-ink cue following, and the span fallback on older WebViews. Next open-to-text performance target: WebView prewarm after bookshelf first paint.
