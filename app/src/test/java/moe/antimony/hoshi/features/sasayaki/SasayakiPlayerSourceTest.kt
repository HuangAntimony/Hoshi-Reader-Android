...
class SasayakiPlayerSourceTest {
    ...
    fun importedAudioUsesPersistedUriInsteadOfPrivateCopyLikeIos() {
        ...
        assertTrue(importAudio.contains("audioCommands.importAudio("))
        assertTrue(importAudio.contains("audioUri = audioUri"))
        assertTrue(importAudio.contains("copiedAudioFileName = copiedAudioFileName"))
        assertTrue(importAudio.contains("teardownPlayer = ::teardownPlayer"))
        assertTrue(importAudio.contains("restoreAudio = ::restoreAudio"))
        assertFalse(importAudio.contains("playbackPersistence.importAudio(audioUri, copiedAudioFileName)"))
        ...
        assertTrue(restoreAudio.contains("callbacks = audioRestoreCallbacks.build("))
        assertTrue(restoreAudio.contains("updateMediaSession = ::updateMediaSession"))
        assertTrue(restoreAudio.contains("handleSeekComplete = ::handleSeekComplete"))
        assertTrue(restoreAudio.contains("startPlayback = ::startPlayback"))
        assertTrue(restoreAudio.contains("pausePlayback = { pausePlayback() }"))
        assertTrue(restoreAudio.contains("previousCue = ::previousCue"))
        assertTrue(restoreAudio.contains("nextCue = ::nextCue"))
        assertTrue(restoreAudio.contains("isPlaying = { isPlaying }"))
        assertTrue(restoreAudio.contains("onFailure(audioRestoreResult::handleFailure)"))
        assertTrue(restoreAudio.contains("audioRestoreResult.handleSuccess("))
        assertTrue(restoreAudio.contains("result = result"))
        assertTrue(restoreAudio.contains("currentTime = currentTime"))
        assertTrue(restoreAudio.contains("updateCue = ::updateCue"))
        assertTrue(restoreAudio.contains("updateMediaSession = ::updateMediaSession"))
        assertFalse(restoreAudio.contains("audioAvailability.markRestoreFailed"))
        assertFalse(restoreAudio.contains("audioAvailability.markRestoreSucceeded()"))
    ...
    fun audioCanBeClearedAndDescribesStorageMode() {
        ...
        assertTrue(clearAudio.contains("audioCommands.clearAudio("))
        assertTrue(clearAudio.contains("playback = playback"))
        assertTrue(clearAudio.contains("teardownPlayer = ::teardownPlayer"))
        assertFalse(clearAudio.contains("audioSourceRepository.clearAudioSource(playback, appContext.contentResolver)"))
        assertFalse(clearAudio.contains("releasePersistableUriPermission"))
        assertFalse(clearAudio.contains("playbackPersistence.clearAudioMetadata()"))
        assertFalse(clearAudio.contains("audioUri = null"))
        assertFalse(clearAudio.contains("audioFileName = null"))
        assertFalse(clearAudio.contains("audioAvailability.markAudioCleared()"))
    ...
    fun restoredAudioStaysPausedUntilExplicitPlaybackLikeIos() {
        ...
        assertTrue(startPlayback.contains("currentTime = { currentTime }"))
        assertTrue(startPlayback.contains("updateMediaSession = ::updateMediaSession"))
        assertTrue(startPlayback.contains("redisplayCue = { time -> updateCue(time, forceDisplay = true) }"))
        assertFalse(startPlayback.contains("playbackCommands.start("))
    ...
    fun startPlaybackRedisplaysCurrentCueLikeIos() {
        ...
        assertTrue(startPlayback.contains("currentTime = { currentTime }"))
        assertTrue(startPlayback.contains("redisplayCue = { time -> updateCue(time, forceDisplay = true) }"))
        assertFalse(startPlayback.contains("markPlayedOnce = cuePresentation::markPlayedOnce"))
        assertFalse(startPlayback.contains("updateCue(currentTime, forceDisplay = true)"))
    ...
    fun autoScrollCrossChapterCueClearsDisplayedCueBeforeLoadingChapter() {
        ...
        assertTrue(source.contains("private val cueUpdate = SasayakiCueUpdateCoordinator("))
        assertTrue(updateCue.contains("cueUpdate.update("))
        assertFalse(updateCue.contains("currentChapterIndex = getCurrentChapterIndex()"))
        assertFalse(updateCue.contains("autoScroll = cuePresentation.autoScroll"))
        assertFalse(updateCue.contains("hasPlayedOnce = cuePresentation.hasPlayedOnce"))
    ...
    fun playerUsesPlaybackEngineBoundaryForMediaPlayerOperations() {
        ...
        assertTrue(restoreAudio.contains("callbacks = audioRestoreCallbacks.build("))
        assertTrue(restoreAudio.contains("handleSeekComplete = ::handleSeekComplete"))
        assertFalse(restoreAudio.contains("SasayakiAudioRestoreCallbacks("))
        assertFalse(restoreAudio.contains("onCompletion = {"))
        assertFalse(restoreAudio.contains("playbackCommands.mediaSessionSeek("))
        ...
        assertFalse(restoreAudio.contains("playbackState.updateDuration(result.durationMs)"))
        assertTrue(tick.contains("playbackTick.tick("))
        ...
        assertTrue(teardown.contains("clearCue = clearCue"))
        assertTrue(teardown.contains("pausePlayback = { pausePlayback() }"))
        assertTrue(teardown.contains("applyCueDisplayAction = ::applyCueDisplayAction"))
        assertFalse(teardown.contains("playbackLifecycle.releaseEngine()"))
