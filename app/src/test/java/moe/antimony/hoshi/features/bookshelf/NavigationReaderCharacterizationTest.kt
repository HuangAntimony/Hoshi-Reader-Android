...
class NavigationReaderCharacterizationTest {
    ...
    fun readerEntryRestoresBookmarkAndClosesBackToBookshelfState() {
        ...
        assertTrue(bookshelf.contains("onOpenReader(readerOpenRequest(file, parsedBook, loadedBookmark))"))
        assertTrue(bookshelf.contains("onOpenReader(readerOpenRequest(root, parsedBook, loadedBookmark))"))
        assertTrue(appShell.contains("readerSessions = readerSessions + (request.bookId to request)"))
        assertTrue(appShell.contains("backStack.add(AppRoute.ReaderRoute(request.bookId))"))
        assertTrue(readerBranch.contains("ReaderWebView("))
        assertTrue(readerBranch.contains("book = session.book"))
        assertTrue(readerBranch.contains("bookRoot = session.bookRoot"))
        assertTrue(readerBranch.contains("initialChapterIndex = session.bookmark?.chapterIndex ?: 0"))
        assertTrue(readerBranch.contains("initialProgress = session.bookmark?.progress ?: 0.0"))
        assertTrue(readerBranch.contains("onSaveBookmark = { chapterIndex, progress ->"))
        assertTrue(readerBranch.contains("session.onBookmarkChanged(savedBookmark, readingProgress)"))
        assertTrue(readerBranch.contains("bookStorage.saveBookmark(session.bookRoot, savedBookmark)"))
        assertTrue(readerBranch.contains("onClose = ::popRoute"))
        assertTrue(bookshelf.contains("onBookmarkChanged = { savedBookmark, readingProgress ->"))
    }
